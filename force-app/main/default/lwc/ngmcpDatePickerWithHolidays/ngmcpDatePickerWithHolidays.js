...
export default class NgmcpDatePickerWithHolidays extends LightningElement {
    ...
    // 1) Add these new fields at top-level in the class
    @api useFixedPopup = false;        // default off; only turn on in Technical Query
    @track popupStyle = '';            // inline style for fixed positioning
    _repositionBound = null;           // to remove listeners when closing
    ...
    // 13 march replan
    // Replace the plain field with a reactive @api getter/setter

    _minSelectableDate = null;

    @api
    get minSelectableDate() {
        return this._minSelectableDate;
    }
    set minSelectableDate(val) {
        this._minSelectableDate = val; // expect 'YYYY-MM-DD'
        // If parent provides SLA min date, position the calendar to that month/year
        if (val) {
            const [y, m, d] = val.split('-').map(Number);
            this.currentYear = y;
            this.currentMonth = (m ?? 1) - 1;
        }
        // Rebuild calendar if component has been initialised
    ...
    connectedCallback() {
        ...
        // ✅ ENQUIRY REPLAN ONLY:
        // If minSelectableDate exists AND workRequestFlag is false,
        // open calendar on the first valid (D+5) month instead of today
        if (this._minSelectableDate && !this.workRequestFlag) {
            const [year, month] = this._minSelectableDate.split('-').map(Number);
            this.currentYear = year;
            this.currentMonth = month - 1; // JS months are 0-based
        } else {
        ...
        if (this.workRequestFlag) {
            // CWR / Technical Query behaviour (unchanged)
            ...
            // Non-CWR behaviour unchanged
            this.applyUkCutoffMinDateIfNeeded();
            this.buildCalendar();
        }
    }
    get computedPopupStyle() {
        return this.useFixedPopup ? this.popupStyle : null;
    }
    ...
    renderedCallback() {
        ...
        if (this.isCalendarOpen && this.useFixedPopup) {
            this.positionPopupFixed();
        }
    ...
    closeCalendar() {
        this.isCalendarOpen = false;
        this.removeOutsideClickHandler();
        this.popupStyle = '';         // reset any inline style
        this.detachedListeners();     // clean up scroll/resize listeners
    }
    positionPopupFixed() {
        try {
            const input = this.template.querySelector('.input-wrapper');
            const popup = this.template.querySelector('.calendar-popup');
            if (!input || !popup) return;

            const rect = input.getBoundingClientRect();
            const top = rect.bottom + 6;                      // small gap under input
            const left = rect.left;
            const width = Math.max(rect.width, 280);          // min width if you prefer

            this.popupStyle = `position:fixed; top:${top}px; left:${left}px; width:${width}px; z-index:10000;`;

            // Keep aligned while user scrolls/resizes
            if (!this._repositionBound) {
                this._repositionBound = () => {
                    ...
                    this.popupStyle = `position:fixed; top:${t}px; left:${l}px; width:${w}px; z-index:10000;`;
                };
                window.addEventListener('scroll', this._repositionBound, true);
                window.addEventListener('resize', this._repositionBound, true);
            }
        } catch (e) {
            // eslint-disable-next-line no-console
            console.error('Failed to position fixed popup', e);
        }
    }
    ...
    toggleCalendar(event) {
        ...
        } else if (this.useFixedPopup) {
            // Ensure we compute position right after it opens
            requestAnimationFrame(() => this.positionPopupFixed());
        } else {
            // (Optional quality-of-life) Nudge the nearest scroller to reveal the popup
        ...
        if (this.marketSectorCode === 'I' && !this.workRequestFlag) {
            ...
            const earliest =
                ukNow.getHours() >= 17
                    ...
                    : ukMidnight;
            this._minSelectableDate = this.toISODate(earliest);
        }
        console.log('It is exit');
    }

    // Build calendar and mark disabled dates
    async buildCalendar() {
        // 🔹 Re-assert UK cutoff rule (no-op unless sector === 'I' && !workRequestFlag)
        // Now it sets this._minSelectableDate (safe), not this.minSelectableDate (recursive)
        this.applyUkCutoffMinDateIfNeeded();
        ...
        const minIso = this._minSelectableDate || this.minSelectableDate;
    ...
    handleManualInput(event) {
        ...
        if (this._minSelectableDate && this.toISODate(dateObj) < this._minSelectableDate) {
            this.errorMessage = 'Date cannot be earlier than the allowed minimum.';
            return;
        }
    ...
    async initAutoWorkingDate() {
        const today = new Date();
        console.log('workingDayOffset', this.workingDayOffset)
        ...
        console.log('minSelectableDate', this.minSelectableDate)
