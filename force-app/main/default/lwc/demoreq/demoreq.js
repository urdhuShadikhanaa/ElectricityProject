...
// 4 feb change for thirdparty and meter
  ...
  jobTypeSubtypes = {
    ...
    "Other Visits": [
      ...
      { label: "Service Engineer Hire", id: "OtherVisits_ServiceEngineerHire", description: "Request to hire service engineer" },
  ...
  get dynamicQuestions() {
    ...
    const readonlyKeysYesFlow = new Set(['peakLoad', 'MeterSize', 'paymentMethod', 'outletPressure']);
    ...
    return questions.map((q) => {
      ...
      return {
        ...
        disabled: (this.isLocked || q.disabled === true || (yesFlowActive && q.key === 'paymentMethod') || (reconLock && q.key === 'isReconnection') || (lockSame && (q.key === 'paymentMethod' || q.key === 'requiredPaymentType')) || (lockInverse && (q.key === 'newMeterPaymentMethod')) || (resYes && q.key === 'paymentMethod')
          || (resNoNonU6 && q.key === 'paymentMethod')),
        // Force read-only for the four keys when yes-flow is active
        readOnly: (this.isLocked || q.readOnly === true || (yesFlowActive && readonlyKeysYesFlow.has(q.key)) || (resYes && q.key === 'paymentMethod')
          || (resNoNonU6 && q.key === 'paymentMethod'))
          ...
          reader.onload = () => {
            ...
            resolve({
              ...
              payload: {
                ...
                type: file.type,
                size: file.size,
                dType: bucketKey === 'gt1File' ? 'GT1' : 'PHT',
                base64
              }
            });
          };
...
this.gt1InlineVisible = hasAnyFailure && showUiMsg;
  ...
  // ðŸ‘‡ NEW: return rows with a 'rowClass' already computed
