...
export default class NGMCP_HomeScreenTabWizard extends LightningElement {
     // assets
    logoUrl = LOGO;
    ...
    @track isReportsActive = false; //reports submenu collapsed initially
     showReportTab = false;
     showUserTab = false;
     showFilesTab = false;
    connectedCallback() {
        // ensure Requests submenu opens if the active menu is a Requests item
        if (this.activeMenu === 'createRequest' || this.activeMenu === 'viewRequest' || this.activeMenu === 'requests' || this.activeMenu === 'bulkCreate') {
    ...
    get isBulkCreateActive() { return this.activeMenu === 'bulkCreate'; }
    ...
    get bulkCreateClass() { return this.activeMenu === 'bulkCreate' ? 'submenu active' : 'submenu'; }
    ...
    handleMenuClick(event) {
        ...
        } else if (menuId === 'createRequest' || menuId === 'viewRequest' || menuId === 'bulkCreate') {
            this.isRequestOpen = true;
        } else {
        ...
        if (menuId === 'viewRequest') {
         ...
         // control Reports submenu visibility:
        // - clicking Reports toggles it
        // - clicking a Reports submenu ensures it stays open
        // - clicking other menus closes it
        if (menuId === 'reports') {
    ...
    get contentToDisplay() {
        switch (this.activeMenu) {
            ...
            case 'bulkCreate': return 'Bulk Create form goes here.'; 
            default: return '';
        }
    }
    setTabVisibility(){
