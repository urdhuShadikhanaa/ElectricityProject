...
import InstallDate from "@salesforce/schema/Asset.InstallDate";
export default class NgSideTabWizard extends NavigationMixin(LightningElement) {
    ...
    try {
      ...
      members.forEach((member) => {
        ...
        if(this.selectedCode){
          ...
          (cust) =>
            cust.ngme_effective_to_date?.trim() === "9999-12-31T00:00:00+00:00" && cust.customer === this.selectedCode
        );
        }else{
          validCustomers = customers.filter(
          (cust) =>
        ...
        const meterType = validasset.length > 0 ? validasset[0].assettype : "";
        const manufactore =
          validasset.length > 0 ? validasset[0].manufacturer : "";
        const yearofmanufacture =
          validasset.length > 0 ? validasset[0].ngme_yom : "";
        
        const installdate =
          validasset.length > 0 ? validasset[0].installdate : "";
        const paymentMechanism =
          validasset.length > 0 ? validasset[0].ngme_pymttype : "";
        const mesauringcapacity =
          validasset.length > 0 ? validasset[0].ngme_measurecapacity : "";
        const plusddialcount =
          validasset.length > 0 ? validasset[0].plusddialcount : "";

             
        

        console.log("meterType", meterType);
        console.log("manufactore", manufactore);
        console.log("yearofmanufacture", yearofmanufacture);
        console.log("installdate", installdate);
        console.log("paymentType", validasset[0].ngme_pymttype);
        console.log("mesauringcapacity", mesauringcapacity);
        console.log("plusddialcount", plusddialcount);
        console.log("ngme_c_loccode >",members.ngme_c_loccode);
        console.log('postal code', members.ngme_c_spc);
        console.log('postal town', members.ngme_c_st);
        ...
        const combined = validCustomers.map((cust) => ({
          ...
          Address: `${member.ngme_c_sitebn}, ${member.ngme_c_bldngname}, ${member.ngme_c_prncpstreet}, ${member.ngme_c_depndlocality}, ${member.ngme_c_spc},${member.ngme_c_st}`,
          Model: modelNUmber,
          MSN: serialNumber,
          MeterModel: cust.model,
          MeterType: meterType,
          serialNo: serialNumber,
          manufacturer: manufactore,
          yearofmanufacture: yearofmanufacture,
          location: member?.ngme_c_loccode,
          InstallDate: installdate,
          paymentMechanism:  validasset[0]?.ngme_pymttype,
          noofdial: plusddialcount,
          measuringCapacity: mesauringcapacity,
      ...
      this.isModalOpen =
        this.filteredRecords.length > 1 || this.filteredRecords.length === 0;

      console.log("Filtered Records:", this.filteredRecords);
      console.log("Modal Open:", this.isModalOpen);
      if (this.filteredRecords.length === 1) {
        console.log("Selected Record:", JSON.stringify(this.filteredRecords));
        const baseUrl = window.location.origin;
        const url = `${baseUrl}/customerportal/assetdetail`;
        console.log("Redirecting to:", url);
        const jsonString = JSON.stringify(this.filteredRecords);
        console.log("JSON String:", jsonString);
        const encodedData = btoa(jsonString); // base64 encode
        console.log("encodedData:", encodedData);
