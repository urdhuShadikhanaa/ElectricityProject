...
export default class NGMCP_workRequestComponent extends LightningElement {
  ...
  set status(value) {
    ...
    // 4 march end
    // 12 Mar — Meter Move: if Residential, clear photos for Reposition/Relocation
    if (this._status === 'Residential'
      && (this.selectedSubType?.id === 'MeterMove_Reposition'
        || this.selectedSubType?.id === 'MeterMove_Relocation')) {

      ['existingMeterPhotos_Reposition', 'existingMeterPhotos_Relocation'].forEach(k => {
        if (this.uploadedFilesByKey?.[k]) this.uploadedFilesByKey[k] = [];
        if (this.uploadedFilePayloadByKey?.[k]) this.uploadedFilePayloadByKey[k] = [];
      });
      // keep “allUploadedPayloads” in sync if you use it on submit
      if (typeof this.recomputeAllUploadedPayloads === 'function') {
        this.recomputeAllUploadedPayloads();
      }
      this._rerenderNudge = Date.now(); // refresh UI
    }
  }
  ...
  parseGt1DocFields(text) {
    ...
    let mSpev =
  /SPEV\s*[vV]?\s*(?:nom\s*\/\s*max|nom\/max|nom\s*max|max)\s*[\s:*=–—-]{0,4}[\s.]*([0-9]*\.?[0-9]+[0-9,.\s]*)\s*kwh/i
    .exec(scope) ||
  /SPEV\s*[vV]?\s*(?:nom\s*\/\s*max|nom\/max|nom\s*max|max)\s*[\s:*=–—-]{0,4}[\s.]*([0-9]*\.?[0-9]+[0-9,.\s]*)\s*kwh/i
    .exec(text);
    ...
    if (!mSpev) {
      ...
      mSpev = /([0-9]*\.?[0-9]+[0-9,.\s]*)\s*kwh/i.exec(win);
    }
  ...
  scanSpevFromBinary(binStr) {
    ...
    const re =
      /S[\s\S]*?P[\s\S]*?E[\s\S]*?V[\s\S]*?(?:[\s\S]*?[vV])?[\s\S]*?n[\s\S]*?o[\s\S]*?m[\s\S]*?\/[\s\S]*?m[\s\S]*?a[\s\S]*?x[\s\S]*?[\s:*=–—-]{0,4}[\s\S]*?([0-9]*\.?[0-9]+[0-9,.\s]*)[\s\S]*?k[\s\S]*?w[\s\S]*?h/i;
  ...
  questionConfig = {
    ...
    "Remove_Meter": [
      ...
      // 3/12 modify by adil
      {
        label: "Please provide the purging certificate(Optional)",
        type: "file",
    ...
    "OtherVisits_PressureChange": [
      ...
      {
        label:
          "Please attach the GT1 document or Service Quotation",
        type: "file",
        key: "serviceQuotation",
      },
    ],
  ...
  handleJobSelect(event) {
    ...
    if (this.validateThirdPartyJobGuards()) {
      return;
    }
  ...
  validateThirdPartyJobGuards() {
    const jobType = (this.selectedJob || '').trim().toLowerCase();
    const isThirdParty = this.thirdParty;

    const allAddressFilled = this.checkAllAddressFieldsFilled();
    const allAssetFilled = this.checkAllAssetFieldsFilled();
    // Job type rules (NO subtype)
    const jobRules = {
      install: { address: true, asset: false },
      remove: { address: true, asset: true },
      exchange: { address: true, asset: true },
      'meter move': { address: true, asset: true },
      'other visits': { address: true, asset: true }
    };

    const rule = jobRules[jobType];
    const isInvalid =
      isThirdParty &&
      rule &&
      (
        (rule.address && !allAddressFilled) ||
        (rule.asset && !allAssetFilled)
      );
    if (isInvalid) {
      this.dispatchEvent(
        new CustomEvent('statereponse', {
          detail: {
            state: this.state,
            appointmentFlag: true,
            jobtype: this.selectedJob,
            subJobType: this.jobSubType, // kept for backward compatibility
            sla: '',
            successprice: false
          },
          bubbles: true,
  ...
  checkAllAddressFieldsFilled() {
    ...
    const postCodeVal = map.postCode;
    const isPostCodeValid =
      !!postCodeVal && this.isValidUKPostcode(postCodeVal);

    return hasAllRequired && hasOneBuilding && isPostCodeValid;
  }
...
if (!this.validateCombinedAttachmentSize()) {
    ...
    try {
      ...
      this.state = {
        ...this.state,
        serviceType: result?.NGMCP_MX_SERVICETYPE__c
      };
  ...
  if (!key) {
    console.error('Missing data-key on drop zone');
    return;
  }

  this.processFiles(files, key);
}
  ...
  processFiles(fileList, bucketKey) {
          ...
          reader.onload = () => {
            ...
            resolve({
              ...
              payload: {
                ...
                type: file.type,
                size: file.size,
                dType: (bucketKey === 'gt1File' || bucketKey === 'serviceQuotation') ? 'GT1' : bucketKey === 'purgingCertificateFile' ? 'PUR' : 'PHT',
                base64
              }
            });
          };
    ...
    Promise.all(readPromises).then(results => {
      ...
      if (bucketKey === 'gt1File' && GT1_VALIDATION_SUBTYPES.has(this.selectedSubType?.id)) {
        ...
        minValidDate.setDate(minValidDate.getDate() - 90);
        ...
        for (let i = 0; i < payloads.length; i++) {
          ...
          try {
            ...
            if (!validDate) {
              ...
              if (d < minValidDate) {
              isExpired = true;
              errs.push('GT1 valid date must be within the last 90 days (or in the future).');
           ...
           } else if (hasPeak && peak > spev) {
              isKwhMismatch = true;
              errs.push(`Peak Hourly Load (${peak}) must be less than or equal to Energy Required SPEV (${spev}).`);
            } else if (!hasPeak) {
  ...
  // 17 march
  // 12 Mar — Recompute the consolidated payload after bucket edits
  recomputeAllUploadedPayloads() {
    const buckets = this.uploadedFilePayloadByKey || {};
    const merged = Object.keys(buckets).reduce((arr, k) => {
      const items = Array.isArray(buckets[k]) ? buckets[k] : [];
      return arr.concat(items);
    }, []);
    this.allUploadedPayloads = merged;
  }
  // 17 march end
 ...
 handleFileDelete(event) {
    ...
    // Find the payload being deleted
    const deletedPayloads = payloadBucket.filter(f => f.name === name);

    // Remove from bucket collections
    this.uploadedFilesByKey = {
        ...this.uploadedFilesByKey,
    ...
    // Remove from master payload collection
    this.allUploadedPayloads = this.allUploadedPayloads.filter(
        payload => !deletedPayloads.some(
            deleted => deleted.name === payload.name
        )
    );
    this.state = {
        ...this.state,
    ...
    this.errors[key] =
        (this.hasTriedGetPrice && nowEmpty)
   ...
   getTotalUploadedFileSize() {
    return (this.allUploadedPayloads || []).reduce(
